...
arrayid <- as.numeric(args[14])
min.mac <- as.numeric(args[15])
max.maf <- as.numeric(args[16])
min.rv.num <- as.numeric(args[17])
max.rv.num <- as.numeric(args[18])
max.rv.num.prefilter <- as.numeric(args[19])
sliding_window_length <- as.numeric(args[20])
QC_label <- args[21]
variant_type <- args[22]
geno_missing_imputation <- args[23]
...
if(nullobj.file == "NO_NULL_OBJ") {
  ...
  phenotype_list <- unlist(strsplit(phenotype, ","))
  obj_nullmodel_list <- list()
  
  for (pheno.num in phenotype_list) {
    cat("Fitting nullmodel for phenotype:", pheno.num, "\n")
    
    pheno.value <- unique(pheno[, pheno.num])
    is.binary <- FALSE
    ...
    formula <- if (covariates == "NO_COVAR") paste(pheno.num, "~ 1") else paste(pheno.num, "~", paste(unlist(strsplit(covariates, ",")), collapse = " + "))
    nullobj <- fit_nullmodel(as.formula(formula), 
                             data = pheno, 
                             kins = kins, 
                             id = pheno_id, 
                             random.slope = if (random_time_slope == "NO_RANDOM_TIME_SLOPE") NULL else random_time_slope, 
                             groups = if (het_vars == "NO_HET_VARS") NULL else het_vars, 
                             family = family, 
                             verbose = TRUE)
    obj_nullmodel_list[[pheno.num]] <- nullobj
  }
  rm(pheno, pheno.num, kins); gc()
} else {
  ...
  cat("\tTime variable in random slope longitudinal models:", random_time_slope, "\n")
  ls.tmp <- ls()
  obj_nullmodel_list <- get(load(nullobj.file))
  
  for (pheno.num in 1:length(obj_nullmodel_list)) {
    nullobj <- obj_nullmodel_list[[pheno.num]]
    
    if (class(nullobj) == "GENESIS.nullMixedModel") {
      nullobj$sample.id <- row.names(nullobj$model.matrix)
      obj_nullmodel_list[[pheno.num]] <- genesis2staar_nullmodel(nullobj)
      rm(nullobj, pheno.num); gc()
    }
  }
}
if(test.type == "Null") {
  save(obj_nullmodel_list, file = paste0(outfile, ".Rdata"))
  ...
  cat("\tMinimum minor allele count to be included for single variant test:", min.mac, "\n")
  cat("\tSliding window size (bp) to be used in sliding window test:", sliding_window_length, "\n")
  rm(list=setdiff(ls(), c("outfile", "obj_nullmodel_list", "agds.file", "max.maf", "min.rv.num", "max.rv.num", "max.rv.num.prefilter", "QC_label", "variant_type", "geno_missing_imputation", "Annotation_dir", "Annotation_name_catalog", "Use_annotation_weights", "Annotation_name", "user_cores", "arrayid"))); gc()
  ...
  if(arrayid < group.num)
  {
    sub_seq_id <- ((arrayid - 1)*gene_num_in_array + 1):(arrayid*gene_num_in_array)
  }else
  {
    sub_seq_id <- ((arrayid - 1)*gene_num_in_array + 1):sub_seq_num
  }

  gene_centric_coding_dnanexus <- function(genes_info_chr,kk,chr,genofile,obj_nullmodel_list,rare_maf_cutoff,
                                           ...
                                           Use_annotation_weights,Annotation_name,silent)
  {
    ...
    results <- try(Gene_Centric_Coding_PheWAS(chr=chr,gene_name=gene_name,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=rare_maf_cutoff,
  ...
  retry_mclapply <- function(sub_seq_id, user_cores, max_iter = 10) 
  {
    ...
    for (iter in seq_len(max_iter))
    {
      ...
      tmp_out <- mclapply(sub_seq_id,gene_centric_coding_dnanexus,genes_info_chr=genes_info_chr,chr=chr,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=max.maf,
  ...
  gene_centric_coding_incl_ptv_dnanexus <- function(genes_info_chr,kk,chr,genofile,obj_nullmodel_list,rare_maf_cutoff,
                                                    rv_num_cutoff,rv_num_cutoff_max,rv_num_cutoff_max_prefilter,
                                                    QC_label,variant_type,geno_missing_imputation,
                                                    Annotation_dir,Annotation_name_catalog,
                                                    Use_annotation_weights,Annotation_name,silent)
  {
    gene_name <- genes_info_chr[kk,1]
    results <- try(Gene_Centric_Coding_PheWAS(chr=chr,gene_name=gene_name,category="all_categories_incl_ptv",genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=rare_maf_cutoff,
                                              rv_num_cutoff=rv_num_cutoff,rv_num_cutoff_max=rv_num_cutoff_max,rv_num_cutoff_max_prefilter=rv_num_cutoff_max_prefilter,
  ...
  retry_mclapply <- function(sub_seq_id, user_cores, max_iter = 10) 
  {
    ...
    for (iter in seq_len(max_iter))
    {
      ...
      tmp_out <- mclapply(sub_seq_id,gene_centric_coding_incl_ptv_dnanexus,genes_info_chr=genes_info_chr,chr=chr,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=max.maf,
                          rv_num_cutoff=min.rv.num,rv_num_cutoff_max=max.rv.num,rv_num_cutoff_max_prefilter=max.rv.num.prefilter,
                          QC_label=QC_label,variant_type=variant_type,geno_missing_imputation=geno_missing_imputation,
                          Annotation_dir=Annotation_dir,Annotation_name_catalog=Annotation_name_catalog,
                          Use_annotation_weights=Use_annotation_weights,Annotation_name=Annotation_name,silent=TRUE,mc.cores=current_cores)
  ...
  source("Gene_Centric_Noncoding_PheWAS_preload.R")
  ...
  gene_centric_noncoding_dnanexus <- function(genes_info_chr,kk,chr,genofile,obj_nullmodel_list,
                                              ...
                                              Use_annotation_weights,Annotation_name,silent){
    gene_name <- genes_info_chr[kk,1]
    results <- try(Gene_Centric_Noncoding_PheWAS_preload(chr=chr,gene_name=gene_name,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,
  ...
  retry_mclapply <- function(sub_seq_id, user_cores, max_iter = 10) 
  {
    ...
    for (iter in seq_len(max_iter))
    {
      ...
      tmp_out <- mclapply(sub_seq_id,gene_centric_noncoding_dnanexus,genes_info_chr=genes_info_chr,chr=chr,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,
                          dfPromCAGEVarGene.SNV=dfPromCAGEVarGene.SNV,variant.id.SNV.PromCAGE=variant.id.SNV.PromCAGE,
                          dfPromrOCRsVarGene.SNV=dfPromrOCRsVarGene.SNV,variant.id.SNV.PromrOCRs=variant.id.SNV.PromrOCRs,
                          dfHancerCAGEVarGene.SNV=dfHancerCAGEVarGene.SNV,variant.id.SNV.HancerCAGE=variant.id.SNV.HancerCAGE,
                          dfHancerrOCRsVarGene.SNV=dfHancerrOCRsVarGene.SNV,variant.id.SNV.HancerrOCRs=variant.id.SNV.HancerrOCRs,
  ...
  gene_centric_ncRNA_dnanexus <- function(ncRNA_gene_chr,kk,chr,genofile,obj_nullmodel_list,rare_maf_cutoff,
                                          rv_num_cutoff,rv_num_cutoff_max,rv_num_cutoff_max_prefilter,
                                          QC_label,variant_type,geno_missing_imputation,
                                          Annotation_dir,Annotation_name_catalog,
                                          Use_annotation_weights,Annotation_name,silent)
  {
    ...
    results <- try(ncRNA_PheWAS(chr=chr,gene_name=gene_name,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=rare_maf_cutoff,
                                rv_num_cutoff=rv_num_cutoff,rv_num_cutoff_max=rv_num_cutoff_max,rv_num_cutoff_max_prefilter=rv_num_cutoff_max_prefilter,
                                QC_label=QC_label,variant_type=variant_type,geno_missing_imputation=geno_missing_imputation,
                                Annotation_dir=Annotation_dir,Annotation_name_catalog=Annotation_name_catalog,
                                Use_annotation_weights=Use_annotation_weights,Annotation_name=Annotation_name,silent=silent),silent=TRUE)
  ...
  retry_mclapply <- function(sub_seq_id, user_cores, max_iter = 10) 
  {
    ...
    for (iter in seq_len(max_iter))
    {
      ...
      tmp_out <- mclapply(sub_seq_id,gene_centric_ncRNA_dnanexus,ncRNA_gene_chr=ncRNA_gene_chr,chr=chr,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=max.maf,
                          rv_num_cutoff=min.rv.num,rv_num_cutoff_max=max.rv.num,rv_num_cutoff_max_prefilter=max.rv.num.prefilter,
                          QC_label=QC_label,variant_type=variant_type,geno_missing_imputation=geno_missing_imputation,
                          Annotation_dir=Annotation_dir,Annotation_name_catalog=Annotation_name_catalog,
                          Use_annotation_weights=Use_annotation_weights,Annotation_name=Annotation_name,silent=TRUE,mc.cores=current_cores)
  ...
  cat("\tMinimum minor allele count to be included for single variant test:", min.mac, "\n")
  rm(list=setdiff(ls(), c("outfile", "obj_nullmodel_list", "agds.file", "max.maf", "min.rv.num", "max.rv.num", "max.rv.num.prefilter", "sliding_window_length", "QC_label", "variant_type", "geno_missing_imputation", "Annotation_dir", "Annotation_name_catalog", "Use_annotation_weights", "Annotation_name", "user_cores", "arrayid"))); gc()
  ...
  sliding_window_dnanexus <- function(kk,chr,start_loc,end_loc,sliding_window_length,genofile,obj_nullmodel_list,rare_maf_cutoff,
                                      rv_num_cutoff,rv_num_cutoff_max,rv_num_cutoff_max_prefilter,
                                      QC_label,variant_type,geno_missing_imputation,
                                      Annotation_dir,Annotation_name_catalog,
                                      Use_annotation_weights,Annotation_name,silent)
  {
    ...
    results <- try(Sliding_Window_PheWAS(chr=chr,start_loc=start_loc_sub,end_loc=end_loc_sub,sliding_window_length=sliding_window_length,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=rare_maf_cutoff,
  ...
  out <- mclapply(sub_seq_id,sliding_window_dnanexus,chr=chr,start_loc=start_loc,end_loc=end_loc,sliding_window_length=sliding_window_length,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,rare_maf_cutoff=max.maf,
                  rv_num_cutoff=min.rv.num,rv_num_cutoff_max=max.rv.num,rv_num_cutoff_max_prefilter=max.rv.num.prefilter,
                  QC_label=QC_label,variant_type=variant_type,geno_missing_imputation=geno_missing_imputation,
                  Annotation_dir=Annotation_dir,Annotation_name_catalog=Annotation_name_catalog,
  ...
  cat("\tMaximum number of variants before extracting the genotype matrix:", max.rv.num.prefilter, "\n")
  cat("\tSliding window size (bp) to be used in sliding window test:", sliding_window_length, "\n")
  rm(list=setdiff(ls(), c("outfile", "obj_nullmodel_list", "agds.file", "max.maf", "QC_label", "variant_type", "geno_missing_imputation", "Annotation_dir", "Annotation_name_catalog", "Use_annotation_weights", "Annotation_name", "user_cores", "arrayid"))); gc()

  nullobj <- obj_nullmodel_list[[1]]
  ...
  individual_analysis_dnanexus <- function(kk,chr,start_loc,end_loc,genofile,obj_nullmodel_list,mac_cutoff,subset_variants_num,
                                           QC_label,variant_type,geno_missing_imputation,p_filter_cutoff)
  {
    ...
    results <- try(Individual_Analysis_PheWAS(chr=chr,start_loc=start_loc_sub,end_loc=end_loc_sub,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,mac_cutoff=mac_cutoff,subset_variants_num=subset_variants_num,
  ...
  retry_mclapply <- function(sub_seq_id, user_cores, max_iter = 10) 
  {
    ...
    for (iter in seq_len(max_iter))
    {
      ...
      tmp_out <- mclapply(sub_seq_id,individual_analysis_dnanexus,chr=chr,start_loc=start_loc,end_loc=end_loc,genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,mac_cutoff=min.mac,subset_variants_num=subset_variants_num,
